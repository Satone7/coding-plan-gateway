...
import {
  ...
  OpenAIStreamToAnthropicConverter,
...
export class RequestProxy {
  ...
  /**
   * Forward an OpenAI-format streaming request to an Anthropic-format upstream.
   * Converts the request, forwards as Anthropic, and converts SSE events back to OpenAI chunks.
   */
  async forwardOpenAIStreamAsAnthropic(
    request: ChatCompletionRequest,
    options: ProxyRequestOptions,
    reply: FastifyReply,
    onTokenUsage?: (tokenUsage?: StreamTokenUsage, accumulatedText?: string) => void
  ): Promise<void> {
    const anthropicRequest = convertOpenAIToAnthropicRequest(request);
    await this.forwardAnthropicStreamAsOpenAIImpl(
      anthropicRequest,
      options,
      reply,
      false,
  ...
  /**
   * Forward an Anthropic-format streaming request to an OpenAI-format upstream.
   * Converts the request, forwards as OpenAI, and converts SSE chunks back to Anthropic events.
   */
  async forwardAnthropicStreamAsOpenAI(
    request: AnthropicMessageRequest,
    options: ProxyRequestOptions,
    reply: FastifyReply,
    onTokenUsage?: (tokenUsage?: StreamTokenUsage, accumulatedText?: string) => void
  ): Promise<void> {
    const openaiRequest = convertAnthropicToOpenAIRequest(request);
    await this.forwardAnthropicStreamAsOpenAIImpl(
      openaiRequest,
      options,
      reply,
  ...
  /**
   * Internal implementation for cross-format streaming.
   * @param convertedRequest - The request in the UPSTREAM's format
   * @param upstreamIsOpenAI - true if upstream expects OpenAI format (convert Anthropic→OpenAI events)
   *                           false if upstream expects Anthropic format (convert OpenAI→Anthropic events)
   */
  private async forwardAnthropicStreamAsOpenAIImpl(
    convertedRequest: AnthropicMessageRequest | ChatCompletionRequest,
    options: ProxyRequestOptions,
    reply: FastifyReply,
    upstreamIsOpenAI: boolean,
    onTokenUsage?: (tokenUsage?: StreamTokenUsage, accumulatedText?: string) => void
  ): Promise<void> {
    const basePath = options.baseUrl.endsWith('/')
      ? options.baseUrl.slice(0, -1)
      : options.baseUrl;
    const isHttps = basePath.startsWith('https');
    const requestFn = isHttps ? httpsRequest : httpRequest;
    const timeout = options.timeout ?? DEFAULT_REQUEST_TIMEOUT_SEC;
    ...
    if (upstreamIsOpenAI) {
      // Upstream expects OpenAI format
      const req = convertedRequest as ChatCompletionRequest;
      url = new URL(`${basePath}/v1/chat/completions`);
      headers = buildHeaders(options.apiKey, 'bearer', undefined, true);
      body = req;
    } else {
      // Upstream expects Anthropic format
      const req = convertedRequest as AnthropicMessageRequest;
      const urlPath = basePath.endsWith('/v1') ? '/messages' : '/v1/messages';
      url = new URL(`${basePath}${urlPath}`);
      headers = buildHeaders(options.apiKey, 'x-api-key', {
        'anthropic-version': '2023-06-01',
        'anthropic-dangerous-direct-browser-access': 'true',
      }, true);
      body = req;
    ...
    const converter = upstreamIsOpenAI
      ? new OpenAIStreamToAnthropicConverter()
      : new AnthropicStreamToOpenAIConverter();
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const req = requestFn(url, { method: 'POST', headers, timeout: timeout * 1000 }, (res) => {
        if (res.statusCode && res.statusCode >= 400) {
          let data = '';
          res.on('data', (chunk) => { data += chunk; });
          res.on('end', () => { handleError(`Upstream error: ${res.statusCode} - ${data.slice(0, 500)}`); });
          return;
        }
        ...
        res.on('data', (chunk: Buffer) => {
          // Set SSE headers lazily on first data
          if (!sseHeadersSet) {
          ...
          for (const line of lines) {
            ...
            // Track accumulated text for token usage fallback
            if (upstreamIsOpenAI) {
              // OpenAI upstream: extract content from "choices":[{"delta":{"content":"..."}}]
              const contentMatch = line.match(/"content"\s*:\s*"((?:[^"\\]|\\.)*)"/);
              if (contentMatch) accumulatedText += contentMatch[1]!.replace(/\\n/g, '\n').replace(/\\"/g, '"');
            } else {
              // Anthropic upstream: extract text from delta events
              const textMatch = line.match(/"text_delta".*?"text"\s*:\s*"((?:[^"\\]|\\.)*)"/);
              if (textMatch) accumulatedText += textMatch[1]!.replace(/\\n/g, '\n').replace(/\\"/g, '"');
            }
          }
        });
        ...
        res.on('end', () => {
          ...
          if (sseBuffer) converter.feedLine(sseBuffer);
          if (sseHeadersSet) {
            reply.raw.end();
          }
          resolve();
        ...
        res.on('error', (error) => { handleError(`Stream error: ${error.message}`); });
      });

      converter.onChunk = (data: string) => {
      ...
      converter.onUsage = (usage) => {
        if (onTokenUsage) {
          onTokenUsage(
            { totalTokens: usage.inputTokens + usage.outputTokens, inputTokens: usage.inputTokens, outputTokens: usage.outputTokens },
            accumulatedText,
          );
        }
      };

      req.on('error', (error) => { handleError(`Request failed: ${error.message}`); });
      req.on('timeout', () => { req.destroy(); handleError('Request timeout'); });
      req.write(bodyStr);
      req.end();
    });
  }
